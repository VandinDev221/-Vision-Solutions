  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={product.name}>
      <div className="space-y-6">
        ...
        {/* Description */}
        <p className="text-sm text-slate-300 leading-relaxed bg-[#090d16] p-4 rounded-xl border border-slate-800">
          {product.description}
        </p>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
            {product.features.map((feat, idx) => (
              <div key={idx} className="flex items-start gap-2 text-xs text-slate-300 bg-[#090d16]/80 p-2.5 rounded-lg border border-slate-800/60">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {product.techStack.map((tech, idx) => (
              <span
                key={idx}
                className="text-xs font-mono px-2.5 py-1 rounded-md bg-[#090d16] text-slate-300 border border-slate-800"
              >
                {tech}
              </span>
            ))}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-slate-800">
          ...
          <Link href="/contato" onClick={onClose} className="w-full sm:w-auto">
            <Button variant="primary" size="sm" className="w-full sm:w-auto font-semibold">
